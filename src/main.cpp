...
#if defined(NDEBUG)
# error "Pesobyte cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xf5cccef05d788cb3311abbb3123b2725b85d6fe33e9c0234a17315f14af3dcbc");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Pesobyte: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Pesobyte Signed Message:\n";
...
{
    // Pesobyte: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Pesobyte: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Pesobyte: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Pesobyte: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Pesobyte: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Pesobyte: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x55c3de26a7d42e0f29d417cb6e981ebe559eb2947444270333c0bc26132e7790");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Mexican Peso looses ground to the US Dollar 10/22/2023";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04a5cdca4a85dea4c1eb59339232e5314b233457c31f872ca6d8ef2542acb151a2c17d811c98d6b6c1bd5501741326e5e76acbe7a77ac516a86dcf5d7af87ff6b3") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1698006930;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084816833;

        if (fTestNet)
        {
            block.nTime    = 1698006886;
            block.nNonce   = 385346809;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x83e27de48ee836cf7c15a08c87e7ce0104484631541c0c470a7c9b8beb696ea2"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc1, 0xb3, 0xda }; // Pesobyte: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PesobyteMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PesobyteMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PesobyteMiner(CWallet *pwallet)
{
    printf("PesobyteMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("pesobyte-miner");
    ...
    try { loop {
        ...
        printf("Running PesobyteMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PesobyteMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PesobyteMiner, pwallet));
}
